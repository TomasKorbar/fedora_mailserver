...
run:
	...
	sudo podman run -d --hostname=jerry.second-domain.com -p :53/tcp -p :53/udp -p :25 -p :8891 -p 9900:110 --name=cont-fedora-mailserver-jerry --network mailservers-network-jerrys-domain --cap-add=CAP_NET_ADMIN -t fedora-mailserver-jerry
